...
var changecheck = require('./routes/changecheck');
var findeuser = require('./routes/findeuser');
var findeid = require('./routes/findeid');
...
app.use('/changecheck', changecheck);
app.use('/findeuser', findeuser);
app.use('/findeid', findeid);
app.use('/main', main);
app.use('/win', win);
...
app.use(function(req, res, next){
  res.send('<script type="text/javascript">alert("없는 페이지입니다 로그인페이지로 이동합니다")</script><script>location.href="/login";</script>');
});
app.use(function(err, req, res, next){
  console.log(err);
  res.send('<script type="text/javascript">alert("잘못된 접근입니다 로그인을 하고 이용해주세")</script><script>location.href="/login";</script>');
});
...
io.on('connection', function(socket) {
  ...
  socket.on('call button', function(data) { //send message를 키값으로지정하고 콜백으로 data
    ...
    if (checkCall >= ((data.starterLength - 1) - checkDieMemList.length) && checkCallPhase === 0) {
      ...
      if (checkAgainPoint >= 1) { //최대값을 가지지 못한사람은 다 다이멤버로 포함시켜서 처리한다.
        ...
        console.log('처리후'+socket.handshake.session.user.id+socket.handshake.session.user.score);
  ...
  socket.on('new user', function(data, callback) {
    callback(true);
    socket.username = socket.handshake.session.user.id; //데이터를 socket.username에 넣어준다.
